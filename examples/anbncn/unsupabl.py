...
class sort_KB(UnsupKBBase):
    ...
    def logic_forward(self, nums):
        ...
        cur_count=0
        for _ in range(l):
            if _ >0 and nums[_]<nums[_-1]:
                return False
            elif _>0 and nums[_]>nums[_-1]:
                if count is None:
                    count=cur_count
        ...
        return count is None or cur_count==count
...
def main(args):
    ...
    bridge.train(train_data, epochs=args.epoches, batch_size=10, prior=prior)
    end_time=time.time()
...
def get_args():
    ...
    parser.add_argument("--digit_size", type=int, default=6)
