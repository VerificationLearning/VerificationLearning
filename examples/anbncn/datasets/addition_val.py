...
class DigitsOperator(TorchDataset):
    ...
    def find_factors(self, n):
        factors = set()  # 使用 set 避免重复的因子
        for i in range(1, int(n ** 0.5) + 1):  # 遍历从 1 到 sqrt(n)
            if n % i == 0:
                factors.add(i)  # 添加因子 i
                factors.add(n // i)  # 添加对应因子 n / i
        return sorted(factors)  # 返回排序后的因子列表
    
    def generate_sequences(self, n, m):
        # 随机选择k个数字
        factors = self.find_factors(n)  # 至少选1个数字，最多选m个数字
        
        # 确保 n 能被 k 整除
        # if n % k != 0:
            # raise ValueError("n must be divisible by the number of selected distinct numbers")
        for k in factors:
        # 每个数字出现的次数
            count = n // k
        
        # 随机选择k个数字并排序
            all_numbers=list(itertools.combinations(list(range(m)), k))
            for chosen_numbers in all_numbers: 
                # chosen_numbers = sorted(random.sample(range(m), k))
                # 构造序列
                sequence = []
                for num in chosen_numbers:
                    sequence.extend([num] * count)
                self.all_list.append(sequence)
    ...
    def __init__(
        ...
        rng = random.Random(seed)
        self.generate_sequences(self.size,self.num_classes)
        ...
        try:
            for sequence in sequences:
                ...
                for c in sequence:
                    self.counts[c]+=1
                    inputs.append((random.choice(self.categorized_data[c]),c))
